...
import { RiCloseLine, RiAddLine, RiSubtractLine, RiDeleteBinLine, RiShoppingCart2Line } from 'react-icons/ri';

export default function CartPanel() {
  const { items, isOpen, setIsOpen, updateQuantity, removeItem, clearCart, totalItems } = useCart();
  const [showLoanForm, setShowLoanForm] = useState(false);
  const [loanType, setLoanType] = useState('');
  const [formData, setFormData] = useState({ purpose: '', department: '', start_date: '', end_date: '', location: '' });
    ...
    <>
      ...
      <div className={`cart-panel ${isOpen ? 'open' : ''}`}>
        <div className="cart-header">
          <h2><RiShoppingCart2Line style={{ verticalAlign: 'middle', marginRight: 8 }} /> Cart ({totalItems})</h2>
          <button className="btn btn-icon btn-outline" onClick={() => setIsOpen(false)}>
            <RiCloseLine size={20} />
          </button>
        </div>
              ...
              {items.length === 0 ? (
                ...
                <div key={item.id} className="cart-item">
                  ...
                  <div className="qty-control">
                    <button onClick={() => updateQuantity(item.id, item.quantity - 1)}><RiSubtractLine /></button>
                    <span>{item.quantity}</span>
                    <button onClick={() => updateQuantity(item.id, item.quantity + 1)}><RiAddLine /></button>
                  </div>
                  <button className="btn btn-icon btn-outline" style={{ color: 'var(--error)', borderColor: 'var(--error)', flexShrink: 0, width: 28, height: 28 }} onClick={() => removeItem(item.id)}>
            ...
            {items.length > 0 && (
              <div className="cart-footer">
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button className="btn btn-primary" style={{ flex: 1, background: 'linear-gradient(135deg, var(--temporary), #60a5fa)' }}
                    onClick={() => handleCheckout('temporary')}>
                    ⏱️ Temporary Loan
                  </button>
                  <button className="btn btn-primary" style={{ flex: 1, background: 'linear-gradient(135deg, var(--permanent), #c084fc)' }}
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            <div style={{ flex: 1, overflow: 'auto', padding: 24 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
                <button type="button" className="btn btn-sm btn-outline" onClick={() => setShowLoanForm(false)}>← Back</button>
                <h3 style={{ fontSize: 16 }}>
                  {loanType === 'temporary' ? '⏱️ Temporary' : '📌 Permanent'} Loan Request
                </h3>
              </div>
              ...
              <div className="input-group">
                ...
                <textarea value={formData.purpose} onChange={e => setFormData(p => ({ ...p, purpose: e.target.value }))}
                  placeholder="Why do you need these items?" required />
              </div>
              ...
              {loanType === 'temporary' && (
                <div className="input-group">
                  ...
                  <input type="date" value={formData.end_date} onChange={e => setFormData(p => ({ ...p, end_date: e.target.value }))} required />
                </div>
              )}
                ...
                <>
                  <div className="input-group">
                    ...
                    <input type="text" value={formData.location} onChange={e => setFormData(p => ({ ...p, location: e.target.value }))}
                      placeholder="e.g., Main Sanctuary, Youth Room, Office" required />
                  </div>
                  <div style={{ padding: 12, background: 'var(--warning-bg)', borderRadius: 8, border: '1px solid rgba(245,158,11,0.2)', fontSize: 12, color: 'var(--warning)' }}>
                    ⚠️ Permanent loans require admin approval and items will be marked as deployed to the specified location.
                  </div>
                </>
              )}
            </div>
