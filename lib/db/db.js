...
async function autoSyncFromSheets() {
  ...
  // Fire ALL Sheet reads in parallel — users, templates, loans, and inventory
  const [usersResult, templatesResult, loansResult, inventoryResult] = await Promise.allSettled([
    restoreUsersFromSheet(),
  ...
  if (usersResult.status === "rejected") console.error("User restore failed:", usersResult.reason?.message);
