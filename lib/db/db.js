...
function beginSyncIfNeeded() {
  getDb();
  if (!_needsSync) return null;
...
export async function waitForSync() {
  const syncPromise = beginSyncIfNeeded();
  if (syncPromise) {
    await syncPromise;
  }
}

export function startSyncIfNeeded() {
  return beginSyncIfNeeded();
}
...
async function autoSyncFromSheets() {
  ...
  console.warn("Auto-syncing inventory from Google Sheets (cold start)...");
  ...
  const seedAll = db.transaction(() => {
    ...
    for (let i = 2; i < spareData.length; i++) {
      ...
      const current = clampInventoryCurrent(quantitySpare, quantitySpare);
  ...
  try {
    ...
    if (client) {
      const quantityBySheetRow = new Map();
      ...
      if (approvedLoans && approvedLoans.length > 0) {
        ...
        if (approvedItems && approvedItems.length > 0) {
          for (const item of approvedItems) {
            quantityBySheetRow.set(
              item.sheet_row,
              (quantityBySheetRow.get(item.sheet_row) || 0) + item.quantity,
            );
          }

          const deductApproved = db.transaction(() => {
            for (const [sheetRow, quantity] of quantityBySheetRow.entries()) {
              db.prepare(
                "UPDATE storage_items SET current = MAX(0, current - ?) WHERE sheet_row = ?",
              ).run(quantity, sheetRow);
            }
          });
          deductApproved();
          console.warn(
            `Replayed ${approvedItems.length} approved loan item deductions across ${quantityBySheetRow.size} storage rows from Supabase`,
          );
        }
      }
    }
