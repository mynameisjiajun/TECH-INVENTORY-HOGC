...
async function autoSyncFromSheets() {
  ...
  const seedAll = db.transaction(() => {
    ...
    console.log(`Auto-synced ${spareCount} storage items, ${deployedCount} deployed items`);
  });
  ...
  try {
    ...
    console.error("Failed to replay approved loans on cold start:", err.message);
  }

  console.log(`Cold-start inventory sync completed in ${Date.now() - syncStart}ms`);
}
...
function runStartupMigrations(db) {
  ...
  if (!hasColumn(db, "deployed_items", "loan_request_id")) {
    db.exec("ALTER TABLE deployed_items ADD COLUMN loan_request_id INTEGER DEFAULT NULL");
  }
}
