...
import fs from "fs";
import { readSheet, clearAndWriteSheet } from "@/lib/services/sheets.js";
...
async function autoSyncFromSheets() {
  ...
  console.log("Auto-syncing from Google Sheets (cold start)...");
  const syncStart = Date.now();

  // Users MUST be restored first — loans have a FK to users(id).
  // Running them in parallel causes FK constraint failures and silent data loss.
  const usersResult = await Promise.allSettled([restoreUsersFromSheet()]);
  if (usersResult[0].status === "rejected") {
    console.error("User restore failed:", usersResult[0].reason?.message);
  }

  // Now restore templates, loans, and fetch inventory sheets in parallel
  const [templatesResult, loansResult, inventoryResult] = await Promise.allSettled([
    restoreTemplatesFromSheet(),
    restoreLoansFromSheet(),
    Promise.all([
  ...
  if (templatesResult.status === "rejected") console.error("Template restore failed:", templatesResult.reason?.message);
  if (loansResult.status === "rejected") console.error("Loan restore failed:", loansResult.reason?.message);

  if (inventoryResult.status === "rejected") {
    console.error("Failed to read inventory from Google Sheets:", inventoryResult.reason?.message);
    console.log(`Cold-start sync completed (partial) in ${Date.now() - syncStart}ms`);
    return;
  }

  const [spareData, deployedData] = inventoryResult.value;
  ...
  seedAll();
  console.log(`Cold-start sync completed in ${Date.now() - syncStart}ms`);
}
...
function initializeSchema(db) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS loan_requests (
      ...
      return_photo_url TEXT DEFAULT NULL,
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  ...
  const migrations = [
    ...
    "ALTER TABLE loan_requests ADD COLUMN return_photo_url TEXT DEFAULT NULL",
    ...
    "ALTER TABLE loan_requests ADD COLUMN return_photo_data TEXT DEFAULT NULL",
  ];
  ...
  if (!adminExists) {
    ...
    // NOTE: do NOT sync to Sheets here — restoreUsersFromSheet() runs during
    // autoSyncFromSheets() and would race with this sync, wiping all users.
    // The admin will be synced the next time any user-mutating action calls syncUsersToSheet().
  }
}
...
async function restoreUsersFromSheet() {
  try {
    const data = await readSheet("Users", "A:H");
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      const telegramChatId =
        row[6] != null && String(row[6]).trim() ? String(row[6]).trim() : null;
      ...
      if (existing) {
        ...
        db.prepare(
          "UPDATE users SET password_hash = ?, display_name = ?, role = ?, email = ?, telegram_chat_id = ? WHERE username = ?",
        ).run(passwordHash, displayName, role, email, telegramChatId, username);
      } else {
        // Insert with original ID if possible
        if (id) {
          db.prepare(
            "INSERT OR IGNORE INTO users (id, username, password_hash, display_name, role, email, telegram_chat_id) VALUES (?, ?, ?, ?, ?, ?, ?)",
          ).run(id, username, passwordHash, displayName, role, email, telegramChatId);
        } else {
          db.prepare(
            "INSERT OR IGNORE INTO users (username, password_hash, display_name, role, email, telegram_chat_id) VALUES (?, ?, ?, ?, ?, ?)",
          ).run(username, passwordHash, displayName, role, email, telegramChatId);
        }
      }
      restored++;
    }
...
export async function syncUsersToSheet() {
  ...
  try {
    ...
    const users = db
      .prepare(
        "SELECT id, username, password_hash, display_name, role, email, telegram_chat_id, created_at FROM users",
      )
      .all();
    const header = [
      [
 ...
 * Columns: A=id, B=user_id, C=loan_type, D=status, E=purpose, F=department, G=location, H=start_date, I=end_date, J=admin_notes, K=created_at, L=updated_at, M=Items_JSON, N=return_photo_url
 */
async function restoreLoansFromSheet() {
    try {
        const data = await readSheet("Loan Receipts", "A:N");
        if (!data || data.length < 2) return;
        ...
        const tx = db.transaction(() => {
            const insertReq = db.prepare(`
                INSERT INTO loan_requests (id, user_id, loan_type, status, purpose, department, location, start_date, end_date, admin_notes, created_at, updated_at, return_photo_url)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            `);
            ...
            for (let i = 1; i < data.length; i++) {
                ...
                const returnPhotoUrl = row[13] ? String(row[13]) : null;
                
                insertReq.run(loanId, userId, type, status, purpose, dept, loc, start, end, notes, created || null, updated || null, returnPhotoUrl);
...
export async function syncLoansToSheet() {
    ...
    try {
      const db = getDb();
      const loans = db.prepare("SELECT id, user_id, loan_type, status, purpose, department, location, start_date, end_date, admin_notes, return_photo_url, created_at, updated_at FROM loan_requests ORDER BY created_at ASC").all();
      
      const header = [["ID", "User ID", "Loan Type", "Status", "Purpose", "Department", "Location", "Start Date", "End Date", "Admin Notes", "Created At", "Updated At", "Items JSON", "Return Photo URL"]];
      const rows = [];
      ...
      for(const loan of loans) {
          ...
          rows.push([
              ...
              JSON.stringify(items), loan.return_photo_url || ""
          ]);
      }
