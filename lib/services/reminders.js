...
export async function runReminderJobs(now = new Date()) {
  ...
  const alreadyNotifiedOverdueTech = new Set();
  const alreadyNotifiedOverdueLaptop = new Set();
  const alreadyNotifiedDueSoonTech = new Set();
  const alreadyNotifiedDueSoonLaptop = new Set();
  for (const notification of todayNotifs || []) {
    const laptopOverdueMatch = notification.message.match(/laptop loan #(\d+) is OVERDUE/);
    const techOverdueMatch = !laptopOverdueMatch && notification.message.match(/loan #(\d+) is OVERDUE/);
    const laptopDueSoonMatch = notification.message.match(/laptop loan #(\d+) is due tomorrow/);
    const techDueSoonMatch = !laptopDueSoonMatch && notification.message.match(/loan #(\d+) is due tomorrow/);
    if (laptopOverdueMatch) alreadyNotifiedOverdueLaptop.add(parseInt(laptopOverdueMatch[1]));
    if (techOverdueMatch) alreadyNotifiedOverdueTech.add(parseInt(techOverdueMatch[1]));
    if (laptopDueSoonMatch) alreadyNotifiedDueSoonLaptop.add(parseInt(laptopDueSoonMatch[1]));
    if (techDueSoonMatch) alreadyNotifiedDueSoonTech.add(parseInt(techDueSoonMatch[1]));
  }
  ...
  for (const loan of overdueLoans || []) {
    if (alreadyNotifiedOverdueTech.has(loan.id)) continue;
  ...
  for (const loan of overdueLaptopLoans || []) {
    if (alreadyNotifiedOverdueLaptop.has(loan.id)) continue;
    const loanUser = userMap.get(loan.user_id);
    const laptopList =
      (laptopsByLoan.get(loan.id) || []).map(escapeHtml).join(", ") || "Laptops";
  ...
  for (const loan of dueSoonLoans || []) {
    if (alreadyNotifiedDueSoonTech.has(loan.id)) continue;
    const loanUser = userMap.get(loan.user_id);
    const loanItems = itemsByLoan.get(loan.id) || [];
    const itemList = loanItems
      .map((item) => `${escapeHtml(item.item_name)} × ${item.quantity}`)
  ...
  for (const loan of dueSoonLaptopLoans || []) {
    if (alreadyNotifiedDueSoonLaptop.has(loan.id)) continue;
    const loanUser = userMap.get(loan.user_id);
    const laptopList =
      (laptopsByLoan.get(loan.id) || []).map(escapeHtml).join(", ") || "Laptops";
  ...
  if (pendingSubs?.length) {
    ...
    if (availableSubs.length) {
      ...
      for (const sub of availableSubs) {
        ...
        const safeLaptopName = escapeHtml(laptopName);
