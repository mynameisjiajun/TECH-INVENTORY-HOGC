...
function itemList(items) {
  return items
    .map((i) => `<li>${esc(i.item)} × ${esc(i.quantity)}</li>`)
    .join("");
}
 ...
 */
export async function sendLaptopAvailableEmail({
  to,
  displayName,
  laptopName,
}) {
  await send(
    to,
    `💻 Laptop "${laptopName}" is now available!`,
    layout(
      "#10b981",
      "💻 Laptop Available!",
      `
      <p>Hi <strong>${esc(displayName)}</strong>,</p>
      <p>Great news! The laptop <strong>${esc(laptopName)}</strong> you were waiting for is now available to borrow.</p>
      <p>Head to the app to reserve it before someone else does!</p>
      ${btn(`${APP_URL}/inventory/laptop-loans`, "Browse Laptops")}
    `,
    ),
  );
}

export async function sendNewLoanAdminEmails({
  admins,
  userName,
  loanId,
  loanType,
...
export async function sendLoanPendingEmail({
  to,
  displayName,
  loanId,
  loanType,
  ...
  await send(
    ...
    layout(
      ...
      `
      <p>Hi <strong>${esc(displayName)}</strong>,</p>
      <p>We've received your <strong>${esc(loanType)}</strong> loan request (<strong>#${loanId}</strong>) and it's now pending approval.</p>
      <p><strong>Purpose:</strong> ${esc(purpose)}</p>
      ${remarks ? `<p><strong>Remarks:</strong> ${esc(remarks)}</p>` : ""}
      ${items?.length ? itemsBox(items) : ""}
      <p>You'll be notified once an admin reviews your request.</p>
      ${btn(`${APP_URL}/loans`, "View My Loans")}
    `,
    ),
  );
...
export async function sendLoanModifiedEmail({
  to,
  displayName,
  loanId,
  loanType,
  ...
  const headline = autoApproved
    ? "✅ Loan Updated & Approved"
    : adminModified
      ? "📝 Your Loan Was Updated by Admin"
      : "📝 Loan Updated — Pending Approval";
  const accentColor = autoApproved ? "#22c55e" : "#6366f1";
  const statusNote = autoApproved
    ? "Your updated loan has been auto-approved and is active."
    : adminModified
      ? "An admin has updated your loan."
      : "Your updated loan is now pending approval.";
  await send(
    to,
    `📝 Loan #${loanId} Updated`,
    layout(
      ...
      `
      <p>Hi <strong>${esc(displayName)}</strong>,</p>
      <p>Your <strong>${esc(loanType)}</strong> loan request <strong>#${loanId}</strong> has been updated. ${esc(statusNote)}</p>
      ${items?.length ? itemsBox(items) : ""}
      ${btn(`${APP_URL}/loans`, "View My Loans")}
    `,
    ),
...
export async function sendLoanReturnEmail({
  to,
  displayName,
  loanId,
  items,
  ...
  await send(
    to,
    `✅ Return Confirmed — Loan #${loanId}`,
    layout(
      "#22c55e",
      "✅ Return Confirmed",
      `
      <p>Hi <strong>${esc(displayName)}</strong>,</p>
      <p>Your return for loan <strong>#${loanId}</strong> has been recorded. ${adminReturn ? "An admin has confirmed the return." : "Thank you for returning the items!"}</p>
      ${items?.length ? itemsBox(items) : ""}
      ${photoUrl ? `<p><a href="${esc(photoUrl)}" style="color:#6366f1;">View Return Photo</a></p>` : ""}
      ${btn(`${APP_URL}/loans`, "View My Loans")}
    `,
    ),
  );
