...
export async function sendOverdueEmail({
  ...
  try {
    ...
    if (error) {
      console.error("Resend API Error (Overdue):", error);
    }
  } catch (err) {
...
export async function sendDueSoonEmail({
  ...
  try {
    ...
    if (error) {
      console.error("Resend API Error (Due Soon):", error);
    }
  } catch (err) {
...
export async function sendPasswordResetEmail({ to, displayName, resetUrl }) {
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: FROM_EMAIL,
      to,
      subject: "🔑 Password Reset — Tech Inventory",
      html: `
        <div style="font-family: sans-serif; max-width: 520px; margin: 0 auto; color: #1f2937;">
          <div style="background: #1e1b4b; padding: 24px; border-radius: 12px 12px 0 0; text-align: center;">
            <h1 style="color: #818cf8; margin: 0; font-size: 22px;">Tech Inventory</h1>
          ...
          <div style="background: #f9fafb; padding: 28px; border-radius: 0 0 12px 12px; border: 1px solid #e5e7eb;">
            <p style="color: #6366f1; font-weight: 700; font-size: 18px; margin-top: 0;">🔑 Password Reset Request</p>
            <p>Hi <strong>${displayName}</strong>,</p>
...
export async function sendWelcomeEmail({ to, displayName, username }) {
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: FROM_EMAIL,
      to,
      subject: "👋 Welcome to Tech Inventory!",
      html: `
        <div style="font-family: sans-serif; max-width: 520px; margin: 0 auto; color: #1f2937;">
          <div style="background: #1e1b4b; padding: 24px; border-radius: 12px 12px 0 0; text-align: center;">
            <h1 style="color: #818cf8; margin: 0; font-size: 22px;">Tech Inventory</h1>
          ...
          <div style="background: #f9fafb; padding: 28px; border-radius: 0 0 12px 12px; border: 1px solid #e5e7eb;">
            <p style="color: #10b981; font-weight: 700; font-size: 18px; margin-top: 0;">🎉 Welcome to the team!</p>
            <p>Hi <strong>${displayName}</strong>,</p>
            <p>Your account has been successfully created. You can now log in using your username: <strong>${username}</strong>.</p>
            <p>We're excited to have you on board! You can now request loans and manage your inventory directly from the app.</p>
            <a href="${APP_URL}/login" style="display: inline-block; background: #6366f1; color: white; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: 600; margin-top: 8px;">Log In Now</a>
          </div>
        </div>
      `,
    });
...
export async function sendNewLoanUserEmail({
  to,
  displayName,
  loanId,
  loanType,
  ...
  try {
    const { data, error } = await resend.emails.send({
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 520px; margin: 0 auto; color: #1f2937;">
          ...
          <div style="background: #f9fafb; padding: 28px; border-radius: 0 0 12px 12px; border: 1px solid #e5e7eb;">
            <p style="color: #6366f1; font-weight: 700; font-size: 18px; margin-top: 0;">📝 Request Received</p>
            <p>Hi <strong>${displayName}</strong>,</p>
            <p>We've received your request for a <strong>${loanType}</strong> loan (<strong>#${loanId}</strong>).</p>
            <p><strong>Purpose:</strong> ${purpose}</p>
            <div style="background: #fff; border: 1px solid #e5e7eb; border-radius: 8px; padding: 16px; margin: 16px 0;">
              <p style="font-weight: 600; margin: 0 0 8px 0;">Items Requested:</p>
              <ul style="margin: 0; padding-left: 20px;">
                ${items.map((i) => `<li>${i.item} × ${i.quantity}</li>`).join("")}
              </ul>
            </div>
            <p>The admins will review your request shortly. You'll receive another email once it has been approved or rejected.</p>
            <a href="${APP_URL}/loans" style="display: inline-block; background: #6366f1; color: white; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: 600; margin-top: 8px;">View My Loans</a>
          </div>
        </div>
      `,
    ...
    if (error) console.error("Resend API Error (New Loan User):", error);
  } catch (err) {
    console.error("Failed to send new loan user email:", err.message);
  }
}

export async function sendNewLoanAdminEmail({
  to,
  adminName,
  userName,
  loanId,
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: FROM_EMAIL,
      to,
      subject: `🔔 New Loan Request from ${userName} — #${loanId}`,
      html: `
        <div style="font-family: sans-serif; max-width: 520px; margin: 0 auto; color: #1f2937;">
          <div style="background: #1e1b4b; padding: 24px; border-radius: 12px 12px 0 0; text-align: center;">
            <h1 style="color: #818cf8; margin: 0; font-size: 22px;">Tech Inventory</h1>
          ...
          <div style="background: #f9fafb; padding: 28px; border-radius: 0 0 12px 12px; border: 1px solid #e5e7eb;">
            <p style="color: #f59e0b; font-weight: 700; font-size: 18px; margin-top: 0;">🔔 Pending Approval</p>
            <p>Hi <strong>${adminName}</strong>,</p>
            <p><strong>${userName}</strong> has submitted a new <strong>${loanType}</strong> loan request (<strong>#${loanId}</strong>).</p>
            <p><strong>Purpose:</strong> ${purpose}</p>
            <div style="background: #fff; border: 1px solid #e5e7eb; border-radius: 8px; padding: 16px; margin: 16px 0;">
              <p style="font-weight: 600; margin: 0 0 8px 0;">Items Requested:</p>
              <ul style="margin: 0; padding-left: 20px;">
            ...
            <a href="${APP_URL}/admin" style="display: inline-block; background: #6366f1; color: white; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: 600; margin-top: 8px;">Review Request</a>
          </div>
        </div>
      `,
    });
    
    if (error) console.error("Resend API Error (New Loan Admin):", error);
  } catch (err) {
    console.error("Failed to send new loan admin email:", err.message);
  }
}
