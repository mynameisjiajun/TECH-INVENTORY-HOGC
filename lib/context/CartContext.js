...
export function CartProvider({ children }) {
      ...
      if (saved) {
        ...
        if (Array.isArray(parsed)) {
          ...
          const migrated = parsed
            .map((item) =>
              item._cartType ? item : { ...item, _cartType: "tech" },
            )
            // Drop laptop items whose start_date is in the past
            .filter(
              (item) =>
                !(
                  item._cartType === "laptop" &&
                  item.start_date &&
  ...
  const addItem = useCallback((item) => {
    setItems((prev) => {
      const existing = prev.find(
        (i) => i.id === item.id && i._cartType !== "laptop",
      );
      if (existing) {
        return prev.map((i) =>
      ...
      return [
        ...prev,
        { ...item, _cartType: "tech", quantity: 1, max: item.current },
      ];
    });
  }, []);
  ...
  const updateLaptopDates = useCallback(
    (id, startDate, startDateOld, endDate) => {
      setItems((prev) =>
        prev.map((i) => {
          if (
            i.id === id &&
            i._cartType === "laptop" &&
            i.start_date === startDateOld
          ) {
  ...
  const clearCart = useCallback(() => {
    ...
    // Do NOT close the panel here — CartPanel shows a success screen after
    // submission and needs to stay open so the user sees it.
    try {
