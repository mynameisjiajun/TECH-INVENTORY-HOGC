...
export function getDb() {
  if (!db) {
    ...
    // On Vercel cold starts, auto-sync from Google Sheets if DB is empty
    const itemCount = db
      .prepare("SELECT COUNT(*) as count FROM storage_items")
      .get().count;
    if (itemCount === 0) {
      _syncPromise = autoSyncFromSheets().catch((err) => {
        console.error("Auto-sync from Google Sheets failed:", err.message);
      });
    }
  }
  return db;
...
async function autoSyncFromSheets() {
  ...
  if (!sheetsId || !serviceEmail || !privateKey) return;
