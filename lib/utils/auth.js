...
export function createResetToken(user) {
  return jwt.sign(
    ...
    getJwtSecret() + user.password_hash,
    { expiresIn: "1h" },
  );
}

export function decodeResetTokenUnsafely(token) {
  try {
    const decoded = jwt.decode(token);
    if (!decoded || decoded.purpose !== "password_reset") return null;
    return decoded;
  } catch {
    return null;
  }
...
export function verifyResetToken(token, passwordHash) {
  try {
    const payload = jwt.verify(token, getJwtSecret() + passwordHash);
