...
for (let i = 1; i <= 50; i++) {
  ...
  db.exec(`INSERT INTO notifications (user_id, message, link) VALUES (1, '⚠️ Your loan #${i} is OVERDUE!', '/loans')`);
}
...
function runOptimized() {
  const start = performance.now();
  if (overdueLoans.length === 0) return performance.now() - start;
  ...
  // Create a set of loan ids that already have notifications
