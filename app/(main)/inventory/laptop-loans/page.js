...
const LaptopCard = memo(function LaptopCard({
  laptop,
  loanType,
  startDate,
  endDate,
  ...
  const canBorrow =
    isAvailable &&
    startDate &&
    (loanType === "temporary" ? endDate : true) &&
    !isInCart;
  ...
  const statusLabel = isAvailable
    ? "Available"
    : isBlocked
      ? `Unavailable${laptop.return_date ? ` · Returns ${laptop.return_date}` : ""}`
      : isTempLoaned
...
export default function LaptopLoansPage() {
  ...
  const fetchLaptops = useCallback(
    async (signal) => {
      ...
      try {
        ...
        if (endDate && loanType === "temporary")
          params.set("end_date", endDate);
        else if (startDate && loanType === "permanent")
          params.set("end_date", "9999-12-31");
        ...
        cartItems
          .filter(
            (i) =>
              ...
              i.end_date === (loanType === "temporary" ? endDate : null),
          )
          .map((i) => i.id),
      ),
  ...
  const permLoanedLaptops = tiers.flatMap((t) =>
      ...
      <div className="page-container">
        ...
        {tab === "available" && (
          <>
            ...
            <div
              className={`laptop-content-grid${showReturningSidebar ? " with-sidebar" : ""}`}
            >
              {/* Laptop tiers */}
              <div>
                {fetching ? (
                  ...
                  tiers.map((tier) => {
                    const availableLaptops = tier.laptops.filter(
                      (l) => !l.is_perm_loaned,
                    );
              ...
              {showReturningSidebar && (
                ...
                >
                  ...
                  >
                    ...
                    >
                      <RiCalendarLine style={{ color: "var(--accent)" }} />{" "}
                      Returning Soon
                    </h3>
                    <div
                      style={{
                        ...
                        >
                          ...
                          >
                            {laptop.notify_me ? (
                              ...
                              <RiBellLine size={13} style={{ opacity: 0.35 }} />
                            )}
                            <span style={{ marginLeft: 3 }}>
                              {laptop.notify_me ? "On" : "Notify"}
                            </span>
                          </button>
                        </div>
                      ))}
        ...
        {tab === "perm" && (
          <div>
            ...
            >
              These laptops are permanently assigned and not available for
              general borrowing.
            </p>
            {fetching ? (
              <div className="loading-spinner">
