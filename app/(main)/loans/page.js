...
export default function LoansPage() {
  ...
  const { addItem, addLaptopItem, setIsOpen, setItems, setModifyingLoan } =
    useCart();
  const channelRef = useRef(null);
  const fetchLoansRef = useRef(null);
  ...
  const handleBorrowAgainLaptop = async (loan) => {
    ...
    try {
      ...
      for (const item of loan.laptops || []) {
        ...
        if (
          laptop.availability === "perm_loaned" ||
          laptop.availability === "temp_loaned" ||
          laptop.availability === "blocked"
        ) {
          skipped++;
          continue;
        }
      ...
      if (added > 0) {
        ...
        if (skipped > 0)
          toast.success(
            `${added} laptop(s) added to cart (${skipped} currently unavailable)`,
          );
    ...
    try {
      ...
      for (const loanItem of loan.items) {
        ...
        if (si) {
          ...
          const maxAllowed = isApproved
            ? si.current + loanItem.quantity
            : si.current;
  ...
  // the channel to re-subscribe on every filter keystroke).
  useEffect(() => {
  ...
  const isDueSoon = (loan) => {
    if (
      ...
      return false;
    const diff =
      (new Date(loan.end_date) - new Date(today)) / (1000 * 60 * 60 * 24);
      ...
      >
        {overdue && (
          ...
          >
            ...
            <span>OVERDUE — Please return items or contact an admin</span>
          </div>
        )}
        {dueSoon && (
          <div
          ...
          >
            <RiCalendarLine style={{ flexShrink: 0 }} />{" "}
            <span>Due soon — return by {loan.end_date}</span>
          </div>
        )}
        ...
        {(loan.status === "approved" || loan.status === "pending") && (
          ...
          >
            ...
            <button
              className="btn btn-sm"
              onClick={() =>
                isLaptop ? handleModifyLaptopLoan(loan) : handleModifyLoan(loan)
              }
            ...
            >
              {borrowAgainLoading[loan.id] ? (
                ...
                <>
                  <RiEdit2Line style={{ marginRight: 6 }} />
                  Modify
                </>
              )}
            </button>
        ...
        {(loan.status === "returned" || loan.status === "rejected") && (
          <div className="loan-card-link-row" style={{ marginTop: 12 }}>
            <button
              ...
              onClick={() =>
                isLaptop
                  ? handleBorrowAgainLaptop(loan)
                  : handleBorrowAgain(loan)
              }
              disabled={borrowAgainLoading[loan.id]}
              style={{
                color: "var(--accent)",
        ...
        {fetching ? (
          ...
          bundles.map((bundle) => {
            ...
            const bundleDate = new Date(
              bundle.loans[0].created_at,
            ).toLocaleDateString();
            const sharedPurpose = bundle.loans.every(
              (l) => l.purpose === bundle.loans[0].purpose,
            )
            ...
            <div className="modal-header">
              <div>
                <h2 style={{ marginBottom: 2 }}>
                  Return{" "}
                  {returnModalLoan._loanKind === "laptop"
                    ? "Laptop Loan"
                    : "Loan"}{" "}
                  #{returnModalLoan.id}
                </h2>
                ...
                >
                  {returnModalLoan.items
                    .map(
                      (i) =>
                        `${i.item}${returnModalLoan._loanKind !== "laptop" ? ` ×${i.quantity}` : ""}`,
                    )
                    .join(" · ")}
                </div>
              </div>
