...
export async function POST(request) {
  ...
  if (action === 'read') {
    if (!notification_id || !Number.isInteger(Number(notification_id))) {
      return NextResponse.json({ error: 'Valid notification_id is required' }, { status: 400 });
    }
    db.prepare('UPDATE notifications SET read = 1 WHERE id = ? AND user_id = ?').run(notification_id, user.id);
