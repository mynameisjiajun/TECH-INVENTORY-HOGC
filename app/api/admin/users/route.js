...
export async function POST(request) {
  ...
  if (action === 'reset_password') {
    ...
    db.prepare('INSERT INTO audit_log (user_id, action, target_type, target_id, details) VALUES (?, ?, ?, ?, ?)').run(
      user.id, 'reset_password', 'user', user_id, `Reset password for @${target.username}`
    );
  ...
  if (action === 'change_role') {
    ...
    db.prepare('INSERT INTO audit_log (user_id, action, target_type, target_id, details) VALUES (?, ?, ?, ?, ?)').run(
      user.id, 'change_role', 'user', user_id, `Changed @${target.username} role to ${new_role}`
    );
  ...
  if (action === 'delete_user') {
    ...
    db.prepare('INSERT INTO audit_log (user_id, action, target_type, target_id, details) VALUES (?, ?, ?, ?, ?)').run(
      user.id, 'delete_user', 'user', user_id, `Deleted user @${target.username}`
    );
  ...
  if (action === 'update_user') {
    ...
    if (display_name) {
      db.prepare('UPDATE users SET display_name = ? WHERE id = ?').run(display_name, user_id);
    }
  ...
  if (action === 'set_invite_code') {
    if (!invite_code || invite_code.trim().length < 3) {
      return NextResponse.json({ error: 'Invite code must be at least 3 characters' }, { status: 400 });
    }
    setSetting('invite_code', invite_code.trim());

    db.prepare('INSERT INTO audit_log (user_id, action, target_type, target_id, details) VALUES (?, ?, ?, ?, ?)').run(
      user.id, 'set_invite_code', 'settings', 0, 'Updated the registration invite code'
    );
